...
## inference
RESID=YY
for(i in 1:ncol(FC_X0)){RESID[,i]=FC_Y[,i]-(fkap_N +fkap_S) %*% t(evec_D[,1:K_ind]) %*% FC_X0[,i]}
## Inference 
if (K_ind!=(Non_dimX+1)){SD_kap_inv=diag(eval_D[(Non_dimX+1):K_ind]^(-1))}else{SD_kap_inv=(eval_D[(Non_dimX+1):K_ind]^(-1))}  
SC_kap_D=Z0S%*%t(Z1S)/T    #NOTE: Z1S=t(t(FC_X1) %*% evec_D[,(Non_dimX+1):K_ind]); Z0S=t(t(FC_X0) %*% evec_D[,(Non_dimX+1):K_ind])
SC_D=Z0S%*%t(Z0S)/T  

P_KS=t(evec_D[,(Non_dimX+1):K_ind])
C_resid=(RESID)%*%t(RESID)/T
...
comp1 = ctem - 1.96*sqrt(theta*t(IIF)%*%C_resid%*%IIF/T)
comp2 = ctem + 1.96*sqrt(theta*t(IIF)%*%C_resid%*%IIF/T)
####

{result=1*(0>comp1 & 0<comp2)}
indexx=which(KAPP==kap)

if (TT_sim==TSET[1]){RESULT1[iter,indexx]=result}
if (TT_sim==TSET[2]){RESULT2[iter,indexx]=result}
if (TT_sim==TSET[3]){RESULT3[iter,indexx]=result}
if (TT_sim==TSET[4]){RESULT4[iter,indexx]=result}
}

}
incfac=1
if(iter%%10==0)
{print(round(c(incfac*mean(RESULT1[1:iter,2]),incfac*mean(RESULT2[1:iter,2]),incfac*mean(RESULT3[1:iter,2]),incfac*mean(RESULT4[1:iter,2])),digits=4))}
}
...
###### Results ######
## kappa=0
print(round(c( mean((RESULT1[1:iter,1])),mean((RESULT2[1:iter,1])),mean((RESULT3[1:iter,1])),mean((RESULT4[1:iter,1])), mean((RESULT1_sp[1:iter,1])),mean((RESULT2_sp[1:iter,1])),mean((RESULT3_sp[1:iter,1])),mean((RESULT4_sp[1:iter,1])) ),digits=3))

## kappa=1
print(round(c( mean((RESULT1[1:iter,2])),mean((RESULT2[1:iter,2])),mean((RESULT3[1:iter,2])),mean((RESULT4[1:iter,2])), mean((RESULT1_sp[1:iter,2])),mean((RESULT2_sp[1:iter,2])),mean((RESULT3_sp[1:iter,2])),mean((RESULT4_sp[1:iter,2])) ),digits=3))
