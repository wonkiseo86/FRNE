...
for (iter in 1:maxiter)
{
  ...
  ## Data generation
  R_corr <- diag(lbnumber2); block_size <- 2 + Non_dimX; if (rho != 0) {R_corr[1:block_size, 1:block_size] <- toeplitz(rho^(0:(block_size - 1)))}
  ...
  for (TT_sim in TSET)
  {
    ...
    { 
      ...
      for (bd_val in bdw_list) {
        ...
        M_matrix <- (LBF) %*% (fkap_N + fkap_S) %*% t(evec_D[, 1:K_ind]) 
        B_matrix <- sweep(Y_eig_fn, 2, bb, "*") 
        Diff_sq <- (M_matrix - B_matrix)^2
        ctem_vec <- colSums(0.5 * Diff_sq[1:(nt-1), ] + 0.5 * Diff_sq[2:nt, ]) * dt
        
        comp1 <- sum(ctem_vec)
        comp1N <- sum(ctem_vec[1:Non_dimX])
  ...
  if (iter %% 200 == 0) {
    ...
    for (bd_val in bdw_list) {
     ...
     if (iter %in% c(200, 1000, 2000) && bd_val == bdw_list[1]) {
       ...
       t_labels <- as.vector(outer(paste0("T", 1:5), k_labels, paste, sep="_"))
       names(res_vals) <- c("Sp", "Rho", "dnX", "eSig", "bdw", t_labels)
      } else {
        names(res_vals) <- NULL
      }
      print(res_vals)
...
for (bd_val in bdw_list) {
  ...
  # 
  save_filename <- paste0("results/FRNENEW_KsuppadjBLOCKKAPP_esignal", esignal, "_rho", rho, "_Kscheme", Kscheme, "_Kcutmin", Kval_cut, "_dn", Non_dimX, "_HS_bdw", bd_val, "_defac1", decfac1, ".RData")
  
  save(file=save_filename, 
       RESULT1, RESULT2, RESULT3, RESULT4, RESULT5, 
       RESULT1_sp, RESULT2_sp, RESULT3_sp, RESULT4_sp, RESULT5_sp,
       RESULTK1, RESULTK2, RESULTK3, RESULTK4, RESULTK5, 
       RESULTK1_sp, RESULTK2_sp, RESULTK3_sp, RESULTK4_sp, RESULTK5_sp)
  ...
  cat("[kappa=2] ")
  print(round(c( mean(sqrt(RESULT1[1:maxiter,3])),mean(sqrt(RESULT2[1:maxiter,3])),mean(sqrt(RESULT3[1:maxiter,3])),mean(sqrt(RESULT4[1:maxiter,3])),mean(sqrt(RESULT5[1:maxiter,3])), mean(sqrt(RESULT1_sp[1:maxiter,3])),mean(sqrt(RESULT2_sp[1:maxiter,3])),mean(sqrt(RESULT3_sp[1:maxiter,3])),mean(sqrt(RESULT4_sp[1:maxiter,3])),mean(sqrt(RESULT5_sp[1:maxiter,3])) ),digits=3))
