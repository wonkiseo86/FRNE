function Tef_gw_store = f_kappa_est_ci_GW(kappa, GTemp_Grid, GTemp0, GTemp, GRP_Grid, GRP_Gr0, GRP_Gr, Inv_N_GTemp, Inv_S_GTemp, K_ind, Non_dimY, Non_dimX, TR)
%#####################################################
...
indx = floor(T1 / 2);
GW2_size = 5;
gw_f = mean(Xraw_tem(:,(T1-GW2_size+1):T1),2) - mean(Xraw_tem(:,1:GW2_size),2);
gw_clr = log(mean(Xraw_tem(:,(T1-GW2_size+1):T1),2) ./ geomean(mean(Xraw_tem(:,(T1-GW2_size+1):T1),2))) - ...
         log(mean(Xraw_tem(:,1:GW2_size),2) ./ geomean(mean(Xraw_tem(:,1:GW2_size),2)));
